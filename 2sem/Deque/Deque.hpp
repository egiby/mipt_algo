...
class Deque
{
	...
	{
		size_t index; // **this == *(deque.buffer + index)
		const Deque<Type> * deque;
		
		void change(const typename Iterator::difference_type change_value, size_t &new_index)
		{
			new_index = (static_cast<typename Iterator::difference_type>(new_index + deque->buffer_size) + change_value) & (deque->buffer_size - 1);
		}
	...
	public:
		...
		Iterator(size_t real_index, const Deque<Type> * deque)
		:index(real_index), deque(deque)
		{
		}
		...
		typename Iterator::difference_type operator - (const Iterator &it) const
		{
			...
			else
			{
				...
				if (it.getRealIndex() >= deque->_begin.getRealIndex())
					result += deque->buffer_size - it.index;
					
				if (index >= deque->_begin.getRealIndex())
					result -= deque->buffer_size - index;
	...
	void updateBuffer()
	{
		if (size() + 1 == buffer_size)
			reallocate(buffer_size << 1);
			
		if (size() * 4 < buffer_size && size() != 0)
			reallocate(buffer_size >> 1);
	}
...
public:
	...
	Deque()
	: buffer(new Type[2]), _begin(0, this), _end(0, this), buffer_size(2)
	{
	}
	...
	Deque& operator = (const Deque<Type> &deque)
	{
		...
		buffer = new Type[deque.buffer_size];
		
		buffer_size = deque.buffer_size;
