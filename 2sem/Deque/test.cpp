...
size_t getIndex(size_t left, size_t right) // [l, r)
{
	std::uniform_int_distribution<size_t> Index(left, right - 1);
	return Index(generator);
}

void makePush(Deque<int> &deque, std::deque<int> &std_deque, int value)
{
	if (getIndex(0, 2))
	{
...
TEST_P(DequeTest, TestOfPushAndPop)
{
	...
	{
		makePush(deque, std_deque, getNumber());
...
TEST_P(DequeTest, ManyDifferentEvents)
{
	Deque<int> deque;
	std::deque<int> std_deque;
	...
	for (auto event: events)
	{
		...
		switch (event.type)
		{
			...
			case CHANGE_VALUE:
				...
				break;
			case PUSH:
				makePush(deque, std_deque, event.value);
				break;
			case POP:
				getPop(deque, std_deque, my_value, std_value);
				break;
...
//~ INSTANTIATE_TEST_CASE_P(TestName, DequeTest, ::testing::Values(1, 5, 10, 15, 20, 400, 8000, 160000, 3200000));
INSTANTIATE_TEST_CASE_P(TestName, DequeTest, ::testing::Values(100000, 200000, 300000, 400000, 500000, 600000, 700000, 800000, 900000));
