...
class Graph: public NGraph::IGraph<long long>
{
	...
	{
		long long time_start, time_finish;
	
	public:
		
		Weight(long long time_start, long long time_finish)
...
public:
	...
	void insertEdge(ui32 v, ui32 u, long long time_start, long long time_finish)
	{
		g[v].push_back(NGraph::Edge<long long>(u, new Weight(time_start, time_finish)));
	}
...
int main()
{
	...
	for (int i = 0; i < r; ++i)
	{
		...
		scanf("%d%d%d%d", &v, &t_s, &u, &t_f);
		v--;
		u--;
		
		g.insertEdge(v, u, t_s, t_f);
	}
	
	NDijkstra::Dijkstra<long long> d_object(d, &g);
	
	long long distance = d_object.getDistance(v);
	
	if (distance == g.infinity())
	{
		std::cout << -1 << '\n';
		return 0;
	}
